...
- 预设不模仿特定作者。提示词、留出样例与评测候选由项目原创编写；训练语料改为用户提供并声明为公版或原创的二十五个完整文本单元。
...
- 完整语料随 `style-profile.md` 进入现有场景生成 source paths；作者、版权、URL 等说明不混入模型参考区，简短授权声明留在不参与生成的 `style_session.json` 侧车中。
