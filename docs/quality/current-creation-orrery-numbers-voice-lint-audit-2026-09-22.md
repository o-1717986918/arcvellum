...
- 全量 Python 回归：1488 项通过，1 项跳过；`doctor` 与 CLI help 可运行。
- 前端单测：75 个文件、245 项通过；生产前端构建和桌面资源同步成功。
- Pi Worker：10 个文件、102 项通过；架构审计、模块映射和提示词注册表校验通过，工作区干净。
- 未重装当前正在运行的桌面旧安装包；星仪的源码、真实作品只读投影和认证 API 路由均已验证，但实际已安装窗口需随新版侧车发布后复测。
- 尚未对新提示执行真实模型生成的对白 A/B 评估。当前 `doctor` 报告 Pi Worker 运行时 ready，但模型连接为 `configuration-required`；因此不能把提示词合同测试等同于人物声音的实写成效。
