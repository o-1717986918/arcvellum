...
| `src/literary_engineering_studio_engine/literary` | 194 | Literary domain | `domain package exports` | foundation and task contracts | FastAPI or Provider SDKs |
| `src/literary_engineering_studio_engine/prompting` | 13 | Prompt programs | `prompt registry/compiler` | literary contracts | Provider transport |
...
| `src/literary_engineering_studio/application` | 64 | Studio use cases | `application services` | ports and Engine contracts | API/framework adapters |
...
| `src/literary_engineering_studio/runtimes` | 21 | Agent adapters | `runtimes registry` | Runtime SPI and external SDKs | Engine route implementations |
...
| `src/literary_engineering_studio/observability` | 37 | Events and telemetry | `observability projections` | event contracts | task mutation |
...
| `creative-live` | 18 | 只通过 feature client、共享只读合同或命令总线跨域协作 |
| `delivery` | 4 | 只通过 feature client、共享只读合同或命令总线跨域协作 |
