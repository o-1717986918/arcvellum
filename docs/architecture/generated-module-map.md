...
| `src/literary_engineering_studio_engine/literary` | 176 | Literary domain | `domain package exports` | foundation and task contracts | FastAPI or Provider SDKs |
...
| `src/literary_engineering_studio/automation` | 14 | Campaign control | `automation/controller.py` | application/runtime ports | Engine route implementations |
...
| `src/literary_engineering_studio/runtimes` | 12 | Agent adapters | `runtimes registry` | Runtime SPI and external SDKs | Engine route implementations |
...
| `src/literary_engineering_studio/observability` | 33 | Events and telemetry | `observability projections` | event contracts | task mutation |
...
| `src/literary_engineering_studio/api` | 25 | HTTP/SSE adapters | `router factories` | application use cases | direct project mutation |
...
| `workers/pi-worker/src` | 18 | Bounded Pi Worker | `main.ts / worker.ts` | Pi SDK and task contract | formal project access |
...
| `creative-live` | 13 | 只通过 feature client、共享只读合同或命令总线跨域协作 |
