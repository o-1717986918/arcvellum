...
## 5.4 2026-07-27 W1-Exit 实施回执

W1-Exit 的可重复验收基础设施已经落地：

- Playwright 使用真实 Studio API、Vite 页面和 100/300/1000 场确定性作品夹具；
- 100/300 场覆盖五主题与 book/chapter/scene/character 四焦点组合；
- 每个状态检查主脊、真实关系、章节目录、关系透镜、Canvas 非空像素与关键控件不重叠；
- SSE 测试检查增量场景进入后焦点与多开窗口身份不被重建；
- reduced-motion 测试检查探索、关系、导航和章节目录功能等价；
- 千场夹具保留一次详细展开后的节点完整性、Canvas、平移和指针缩放检查；
- 测试增量场景在 `finally` 中清理，夹具启动时也会核对实际场景文件数，避免测试污染。

实际执行证据：

- 100 场五主题/四焦点 Playwright 矩阵：通过；
- 300 场五主题/四焦点 Playwright 矩阵：通过；
- Client：41 files、128 tests passed；
- 100/300/1000 节点布局与六种构型在单元性能门禁中全部可寻址，千节点构型基线约
  32–61 ms；
- 1000 场详细投影的 Playwright 整页 PNG 取证在本机系统 Edge 上会超过动作时限，
  因此不再把“把千节点页面合成为整页截图”当作产品门禁；千场产品门禁由投影测试、
  Canvas 像素、详细展开和平移缩放共同承担。

结论：W1 所有产品功能和自动化验收合同已经实现，后续不再横向扩展星仪功能。若 CI
提供受控 Chromium，可继续执行千场截图作为非阻断观测项；这不影响 W1 产品退出。
