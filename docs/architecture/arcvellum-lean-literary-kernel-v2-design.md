...
READY
  ...
  -> VERIFYING
  -> REVIEWING?        # 由策略触发
  -> REVISION_NEEDED?  # 按审查证据持续定向修订，直到通过或确认无进展
  -> COMMITTABLE
  -> COMMITTED
...
| `standard` | 信息释放、关系变化、普通冲突 | 一次生成、独立语义审查、按证据修订至通过 |
...
| 自动 Revision | 按确定性证据修订至通过 | 按确定性与文学证据修订至通过 | 按证据修订至通过；不可逆决策仍需审批 |
...
- K3 通过紧凑 Prompt 接入 Pi Worker 创作、审查与证据驱动的持续定向修订；
