...
- 新增 PixiJS 多平面场景、二维相机 state、屏幕锚点投影和输入控制；全景天幕独立使用屏幕空间 GPU shader，视角支持不设上限的 yaw/pitch 旋转。
- 实现 pointer-centered zoom、工作面平移、惯性、fit、focus 和 reduced motion；中键轨道视角直接驱动全景天幕的天球射线。
...
## 18. 本轮实施记录：创作进阶星链与真实全景天幕

状态：已实现并完成浏览器验收。实现范围严格限定在前端表现层，不新增后端字段、不新增 CLI 门禁、不复制第二套创作状态机。

### 18.1 创作进阶镜头

- `client/src/features/orrery/model/creativeProgression.ts` 将现有投影节点按架构、资产、推演、正文、交付五个创作阶段分组。
- 阶段状态只读取 `lifecycle`、`completion_state`、`status` 与现有 `available_actions`；前端不会自行推断任务可执行性，也不会把展示卡片当成解锁条件。
- `CreativeProgressionLayer.vue` 将阶段节点投影为可点击的弧线路径。点击有真实锚定节点的阶段，会复用现有聚焦与节点详情流程；尚未出现的阶段只以低亮度待进入状态展示。
- 阶段卡片位于节点之上但低于节点详情层，使用小型工具标签、衬线标题、状态色和呼吸线条，避免把创作工程信息暴露成裸 JSON。

### 18.2 全景天幕

- `client/src/features/orrery/engine/sky.ts` 移植实验稿的实现方式，使用 Pixi `Mesh + Shader + UniformGroup` 绘制全屏程序化天幕。
- 三层稀疏星野、FBM 星云、银河尘带、极光与轻微闪烁都在 shader 内生成；噪点不再进入 `far / mid / near` 叙事场景层。
- 天幕被插入 `app.stage` 的最底层，故事 viewport、关系线、节点 DOM overlay 继续在其上独立移动。中键视角变化映射为天幕 yaw/pitch，滚轮缩放映射为 FOV，平移只提供极小的背景视差。
- 旧的 `celestialCanopy.ts` 场景星点实现已移除，避免未来维护时误把背景噪点重新挂回叙事节点层。

### 18.3 文字层与验证

- 节点文字采用实验稿中的层级：utility 小标签、衬线标题、暗底描边、低强度字距与辉光；状态与节点事实仍由原有 CSS token 控制。
- 定向模型测试：2 个文件、7 个测试通过；全量前端测试：64 个文件、195 个测试通过。
- 生产构建通过，桌面前端同步与构建校验通过。
- 浏览器验收：1,000 场景可平移、滚轮缩放和中键视角操作；真实画布存在、像素方差有效；1440×900 截图确认全景天幕、五阶段进阶镜头和中文节点信息可见，未出现阶段卡片互相遮挡。
