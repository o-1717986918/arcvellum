...
`ActorMaterial/v1`：每个 beat 至多两种候选，包含具体 `spoken`、`visible_action`、`subtext_effect`；角色 Agent 必须扮演一个明确人物，不能替别的人说话或决定情节。其输入带人物稳定声音、当下声音状态、知识/误知、已确认前一 beat、任务单与禁增事实。角色的语言差异由词域、句法、礼貌边界、回避方式、反讽/幽默及受压变调体现，不复读口头禅。
...
执行顺序：冻结场景输入和文风版本 → 主创导演计划 → 按 beat 顺序角色扮演（下一 beat 可见上一 beat 的候选，但不得假定最终采用）→ 环境描写 → 主创全文 → 现有字数补写、确定性验证、独立审读、提交。候选缓存键包括场景输入、表达投射、文风选择、合同版本、所选模型与上游素材摘要；任一变化使旧缓存失效。候选 JSON 保存在 Studio 数据目录的事务缓存中，绝不直接写入作品目录。缓存只证明**提供过哪些候选**，不能根据文字重合推断主创在语义上采用了它们；主创是否采用需人工/专门审计判断。
...
导演任务单失败时，整套候选退化为单主创写作；某演员或环境写手失败时跳过该候选，其他有效素材仍可使用；全部候选失败时也是单主创。模型输出不会被当作事实授权，角色/环境只拿到场景、已确认来源和局部人物声音，不能调用项目工具。角色候选限制为每个节拍最多两种、每句台词 300 字、动作 220 字；环境候选每段最多 350 字；候选总块超过 16000 字则整体回退，避免把正式正文 Prompt 挤出窗口。正式正文仍受原有硬审查，环境比例、修辞和角色辨识属于生成时的软引导与读者评估，不新增风格硬门禁。
