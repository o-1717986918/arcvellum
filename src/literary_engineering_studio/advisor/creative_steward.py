    ...
    def _run(
        ...
        *,
        ...
        try:
            ...
            _wait_for_decision_idle(client, session_id, model, timeout, cancel_event, self._emit)
            try:
                result = _parse_decision(_last_assistant_text(client.messages(session_id)))
            except (RuntimeError, json.JSONDecodeError):
                # A decision is structured control-plane data.  Retry once with
                # an explicit no-tools repair prompt instead of blocking an
                # otherwise healthy full-auto run on an empty/prose response.
                client.prompt_async(
                    session_id,
                    text=_decision_repair_prompt(choice),
                    model=model,
                    agent="creative-steward",
                )
                self._emit("steward.decision.repair_started", {"session_id": session_id, "model": model})
                _wait_for_decision_idle(client, session_id, model, timeout, cancel_event, self._emit)
                try:
                    result = _parse_decision(_last_assistant_text(client.messages(session_id)))
...
def _decision_repair_prompt(choice: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"selected_option must be exactly one of: {json.dumps(option_ids, ensure_ascii=False)}.\n"
