...
def _initial_prompt(root: Path, budget: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "无关精确数字默认不用。日期、年龄、编号、时长、距离、尺寸、次数、比例或读数只有在人物确实需要这份精度、会改变眼前选择且后文会核验或兑现时才进入规划；普通动作、陈设和停顿用状态、范围或后果表达，不用计件、计次、计时制造伪真实感。",
...
def _window_prompt(
    ...
    return "\n".join([
        ...
        "无关精确数字默认不用。日期、年龄、编号、时长、距离、尺寸、次数、比例或读数仅在它会改变人物当下选择并将在后文核验或兑现时规划；普通动作、陈设和停顿改写为状态、范围或结果。快节奏仍需保留句群层次，收束、余波和关系变化应获得相应的呼吸空间。",
        "## 最近的用户方向\n" + ("\n".join(directions)[-5000:] or "无额外方向"),
        "## 导入来源片段\n" + (source_context or "无导入来源"),
