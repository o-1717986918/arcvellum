        ...
        def stream():
            ...
            while True:
                ...
                with deps.stream_lock:
                    ...
                    if revision != previous_revision:
                        sequence = int(state.get("sequence") or 0) + 1
                        delta = deps.spatial_projection_delta(previous_projection, projection)
                        projection["sequence"] = sequence
                        projection["delta"] = delta
                        projection["motion_events"] = deps.spatial_projection_motion_events(previous_projection, projection, delta)
                        deps.v3_stream_state[stream_key] = {"sequence": sequence, "projection": projection}
                    else:
                        sequence = int(state.get("sequence") or 0)
                        delta = None
                if delta is not None:
                    yield deps.sse("narrative.v3.projection", projection, sequence)
                    sent += 1
                    if limit and sent >= limit:
                        break
                else:
