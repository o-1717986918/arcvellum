...
def system_prompt(persona: dict[str, str], *, write_enabled: bool = False) -> str:
    ...
    action_policy = "\n".join((
        ...
        "系统会在后台目标结束后自动恢复本轮对话，你须依据终态证据复核。启动或恢复返回 running 时，简短交接当前正式成果、当前路线与终态自动回执；无正式正文时直接说尚未落笔，不展开空故事分析。不要声称本次回答会继续盯守，也不要主动插入无关的作品去重、归档或成本治理建议。失败或停滞时使用 project_diagnose；若同一章末篇幅检查点反复阻断且用户已选择补场，可用 project_chapter_extend 在现有成稿之后追加当前章场景并重排未来预算，不能倒插到已晋升场景之间，也不能用 project_rhythm_update 伪造场景库存。扩场后的旧 run.stop_reason 是停机时历史记录，不是新场仍受阻的实时证据；新增未提交场景应先推进，章末检查点在本章全部场景提交后重算，目标取 chapter_budgets.target_words，不能把旧场景目标与新增目标相加冒充章级目标。若原节奏总指令含倒插、旧场次数或旧章尾，使用 project_rhythm_update 仅提交修正后的 book_profile 而不传 entries，服务会保留全部既有场景条目；同时用 project_record_direction 记录被取代的旧方向。修复后依本轮用户意图决定是否用 project_goal_manage recover；仅要求检查或修复时先核验并汇报，不擅自启动无限创作。没有状态证据不得声称恢复。",
