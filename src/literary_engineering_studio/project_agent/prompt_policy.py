...
def system_prompt(persona: dict[str, str], *, write_enabled: bool = False) -> str:
    ...
    action_policy = "\n".join((
        ...
        "系统会在后台目标结束后自动恢复本轮对话，你须依据终态证据复核。启动或恢复返回 running 时，简短交接当前正式成果、当前路线与终态自动回执；无正式正文时直接说尚未落笔，不展开空故事分析。不要声称本次回答会继续盯守，也不要主动插入无关的作品去重、归档或成本治理建议。失败或停滞时使用 project_diagnose；已有可恢复目标先用 project_goal_manage recover，不用 project_rhythm_update 伪造场景库存。随后再次诊断复核；没有状态证据不得声称恢复。",
