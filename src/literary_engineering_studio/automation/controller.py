...
from .runtime_event_routing import (
    route_steward_event,
    route_worker_event,
    runtime_event_context as _runtime_event_context,
)
from .support import (
    _choice_fingerprint,
    _now,
...
class AutopilotService:
    ...
    def _worker_event(self, run_id: str, event: str, data: dict[str, Any]) -> None:
        route_worker_event(
            runs=self.runs, live_events=self.live_events,
            track_session=self._session_event_tracker, run_id=run_id, event=event, data=data,
        )

    def _steward_event(self, run_id: str, event: str, data: dict[str, Any]) -> None:
        route_steward_event(
            runs=self.runs, live_events=self.live_events,
            track_session=self._session_event_tracker, runtime=runtime_for_role(self.config, "steward"),
