"""Archive asset revision table operations."""

from __future__ import annotations

from .archive_primitives import archive_project_key, validate_archive_asset_key
from .primitives import _now


ASSET_REVISION_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS archive_asset_revisions (
    project_root TEXT NOT NULL,
    asset_id TEXT NOT NULL,
    revision TEXT NOT NULL,
    transaction_id TEXT NOT NULL,
    snapshot_path TEXT NOT NULL,
    snapshot_role TEXT NOT NULL,
    created_at TEXT NOT NULL,
    PRIMARY KEY(project_root, asset_id, revision)
);
CREATE INDEX IF NOT EXISTS archive_asset_revisions_transaction_idx
    ON archive_asset_revisions(transaction_id);
"""


def read_asset_revision_tx(
    connection,
    project_root: str,
    asset_id: str,
    revision: str,
) -> dict[str, object]:
    project = archive_project_key(project_root)
    validate_archive_asset_key(asset_id, revision)
    row = connection.execute(
        """
        SELECT project_root, asset_id, revision, transaction_id,
               snapshot_path, snapshot_role, created_at
        FROM archive_asset_revisions
        WHERE project_root = ? AND asset_id = ? AND revision = ?
        """,
        (project, asset_id, revision),
    ).fetchone()
    if row is None:
        raise FileNotFoundError(f"Archive revision not found: {asset_id}@{revision}")
    return dict(row)


def list_asset_revisions_tx(
    connection,
    project_root: str,
    asset_id: str,
    *,
    limit: int = 200,
) -> list[dict[str, object]]:
    project = archive_project_key(project_root)
    validate_archive_asset_key(asset_id)
    rows = connection.execute(
        """
        SELECT project_root, asset_id, revision, transaction_id,
               snapshot_path, snapshot_role, created_at
        FROM archive_asset_revisions
        WHERE project_root = ? AND asset_id = ?
        ORDER BY created_at DESC, revision DESC
        LIMIT ?
        """,
        (project, asset_id, max(1, min(1000, int(limit)))),
    ).fetchall()
    return [dict(row) for row in rows]


def record_asset_revision_tx(
    connection,
    *,
    project_root: str,
    asset_id: str,
    revision: str,
    transaction_id: str,
    snapshot_path: str,
    snapshot_role: str,
    created_at: str,
) -> None:
    validate_archive_asset_key(asset_id, revision)
    connection.execute(
        """
        INSERT OR IGNORE INTO archive_asset_revisions (
            project_root, asset_id, revision, transaction_id,
            snapshot_path, snapshot_role, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (
            archive_project_key(project_root),
            asset_id,
            revision,
            transaction_id,
            snapshot_path,
            snapshot_role,
            created_at or _now(),
        ),
    )
