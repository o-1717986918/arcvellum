...
先遵守 Canon、人物事实和场景目标，再把本场表达计划与所选完整参考转成语言动作。参考提供叙述距离、句法运动、信息隐匿、对白回弹和意象来源。

语言来自视角人物会注意、误解、回避和说出的事物。长短句随压力与认识变化。修辞须改变视角、节奏、关系或信息；情绪可直述，也可由当前最有因果意义的行动、对白、沉默或感知承担。人物的稳定词域与当下对话对象共同决定言语行动。动作、对白或物证已成立时，停在后果。

硬语言边界按项目已编译约束和标点规范执行；软审美风险在生成时自检。
"""
...
写入场景正文候选。遵守 Canon、人物、场景编排、文风生成标准与已编译硬语言边界。
...
def _review_notes_block(
    ...
    return "\n".join(
        [
            ...
            "- 小修应尽量局部：改动作、信息呈现、标点节奏、人物语气或段落收束。",
...
Canon、用户明确约束和人物事实优先。正式生成执行已选择的 composition 分支、场景义务、人物当下声音和 expression plan；旧 composition 的 prose seed 仅为历史证据。文风来源：{_loaded_label(style_path, root, "已加载", "未加载")}；节奏合同：`{rhythm_status}`；预算：{_loaded_label(word_budget_path, root, "已加载", "未加载")}；审读小修：{_loaded_label(review_notes_path, root, "已加载", "未加载")}。

中文标点、项目禁用表达和机械对照按已编译约束审查。软审美问题由表达计划和人物声音在生成时处理。只输出候选正文和状态变化候选；需要偏离已确认事实时写入“需要人工确认”。
"""
