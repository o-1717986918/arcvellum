...
def ensure_scene_pre_generation_tasks_completed(root: Path, scene_id: str) -> None:
    ...
    if not trace.passed:
        raise FlowGateError(
            f"formal context trace must be fresh before generate-scene: {trace.message}. "
            "Rerun the CLI context task, then let the state machine reissue downstream RP, branch, and composition work."
        )
    ...
    for current_state in ("roleplay-agent-task", "composition-agent-task"):
        errors = semantic_artifact_errors(root, current_state, scene_id)
        if errors:
            raise FlowGateError(
                f"formal semantic evidence must pass before generate-scene: {'; '.join(errors[:4])}. "
                "Do not treat a sidecar completion marker as a substitute for the Agent's typed conclusion."
            )
...
def ensure_composition_ready_for_generation(
    ...
    *,
    ...
    if provenance.get("semantic_review_required") is True:
        ...
        if semantic_errors:
            raise FlowGateError(
                "formal composition semantic review required before generate-scene: "
