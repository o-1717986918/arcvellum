...
HTTP model provider entry points are legacy/debug helpers. Formal creative/review commands target the ArcVellum Worker through task sidecars instead of reading provider settings.
...
Expected outputs after the ArcVellum Worker completes `style-prompt` / `style-lab-compile` task sidecars.
...
Generated after the ArcVellum Worker completes `style-prompt-eval`:
...
The candidate is written by the ArcVellum Worker. The deterministic `style-eval` command can then score it against the reference.
...
Default promotion requires an `approve` record whose `run_id` is the candidate id or an explicitly supplied approval id. `--allow-unapproved` is maintainer/debug-only; formal Studio runtimes must not use it.
...
The CLI only stores source text, chunks it, writes a manifest, and writes a Worker task sidecar. It does not infer canon, rewrite character files, choose plot direction, or create final prose.

Expected candidate outputs after the ArcVellum Worker completes the sidecar:
...
These files are diagnostic dashboards and formal route ledgers. They summarize `.agent_tasks.md` sidecars, expected artifact paths, missing outputs, inferred routes, route-specific gates, promotion review gates, unresolved scene review notes when auditing `scene-development`, workflow event/task integrity, and cross-route next actions. `workflow-dashboard` additionally writes `workflow/dashboard/workflow_dashboard.json`, `.md`, and `.html` as a read-only cockpit for ArcVellum Workers or local frontends. They do not create prose, canon, characters, final plot, approval, or workflow completion. A pending sidecar remains pending until the ArcVellum Worker reads it, writes the expected artifacts, checks them, and creates the adjacent `.agent_completion.json` marker.
...
When a promotion manifest exists, `route-audit` also checks whether the promoted candidate had a candidate-specific ArcVellum Worker scene review. The review must cite the exact candidate path in `source_paths`, `candidate`, or `reviewed_candidate`, pass `scene_review.v1`, resolve `new_character_register`, and have no unresolved notes. Manifests that use debug waiver fields such as `allow_unreviewed`, `allow_review_notes`, `allow_unapproved`, or `include_blocked` are blocked for formal Studio routes.
...
`raw_output.md` is the unmodified provider output. `parsed_output.json` is machine-readable only when parsing or later schema validation succeeds. Agent output is evidence, not canon, and must not directly write drafts, character files, releases, or `canon/`. Parsed JSON is still a draft until the ArcVellum Worker reviews it against schema, canon, character, style, and user-intent constraints.
...
- give the ArcVellum Worker executable follow-up tasks;
...
Task files may contain `[AGENT_TASK: ...]`. They are not external LLM prompts, not canon, and not final prose. They are executable work for the ArcVellum Worker that loaded this skill. A CLI command that creates a task file has prepared the next step; it has not completed the creative/review step. The ArcVellum Worker should read the sidecar, write the expected artifacts, inspect them, and only then continue to promotion, readiness, export, or user response. Task files may be deleted, replaced, or converted into formal review notes after the ArcVellum Worker completes the work.
...
It is not final prose. The ArcVellum Worker must interpret the roleplay and consequence notes before selecting, revising, or discarding any branch.
...
It is not final prose. It is the handoff between branch reasoning and scene drafting or provider generation. The ArcVellum Worker must review composition JSON before using it as a prompt pack or writeback source.
...
The prompt manifest records rendered messages, source files, and generation standards for the ArcVellum Worker. It must not contain API keys, `[AGENT_TASK: ...]`, or overwrite canon. The adjacent `.agent_tasks.md` is the execution program: the CLI writes the task, and the main ArcVellum Worker reads it, applies the prompt manifest, and writes the expected candidate Markdown/JSON. Formal prompt manifests require a ready `compose-scene` composition with `selection_source=selection`, `ready_for_generation=true`, and `formal_cli_provenance.created_by=compose-scene`; missing, hand-written, or unselected composition blocks formal generation. Generated prose candidates require a Worker candidate manifest with prompt/task provenance and `new_character_register` before promotion. Generated prose candidates also require Worker review before promotion; run `agent-review-scene --draft <candidate>` or write an equivalent exact-candidate review, then read the generated sidecar and fill the expected `scene_review.v1` JSON/Markdown, including `new_character_register`. Candidate prose must also follow `references/punctuation-standard.md` unless a deliberate exception is recorded.
...
Default promotion is blocked until the exact candidate has passed formal ArcVellum Worker scene review:
...
`--allow-unreviewed` and `--allow-review-notes` are maintainer/debug-only. A formal Studio must not use them, even if the user asks to “unreview” or skip review; complete the exact-candidate review or revision loop instead. If these flags appear in a project manifest, `route-audit` should block formal readiness.
...
`ready` requires a cleaned draft body, context packet, valid context trace, roleplay simulation with Worker reading receipt, resolved roleplay directives, valid branch manifest, formal `branch_selection.md`, ready scene composition, static review `pass`, formal ArcVellum Worker scene review JSON that cites the current draft path, `scene_review.v1` schema validation, clean AgentReview `conclusion=pass`, resolved `new_character_register`, no unresolved `warnings` / `revision_actions` / `style_notes`, and, when a Style Skill is mounted, `style_adherence.status=pass`.
...
`export-package` rebuilds or verifies the chapter workspace before packaging and blocks by default when any scene is non-ready. `--include-blocked` is maintainer/debug-only; formal Studio runtimes must not use it. Final release should have zero skipped scenes and no residual workbench trace warnings.
