...
- `perceptual_options`：只有场景资料确有依据时才列出声音、触感或光线；可为空。
- `expression_plan`：本场叙述距离、信息顺序、句法运动、对白压力和结尾余压的条件，不提供可复制正文句。
- 旧 v0.1 的 `sensory_palette` / `prose_seed` 只作为历史证据读取，不再成为生成措辞义务。
...
它不能在正稿中变成解释性设定段落。`compose-scene` 可以在内部编排文档里显式记录背景影响，后续创作仍遵守“隐性驱动”规则。
...
- 输出包含节拍、潜台词、对白意图、表达计划、可为空的感知选项和写回候选。
