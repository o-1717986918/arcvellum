...
def review_candidate_asset(project_root: Path, candidate: str | Path, provider: str = "auto") -> CandidateReviewResult:
    ...
    # Candidate filenames are allocated by the state machine.  Agent-authored
    # JSON may describe the asset, but it must not rename the formal candidate
    # or redirect deterministic promotion manifests to a different ID/type.
    inferred_type = _infer_type_from_path(root, candidate_path)
    asset_type = _normalize_asset_type(inferred_type or str(payload.get("asset_type") or ""))
...
def promote_candidate_asset(
    ...
    *,
    ...
    candidate_path = _resolve_candidate(root, candidate)
    payload = json.loads(candidate_path.read_text(encoding="utf-8"))
    # Candidate filenames are allocated by the workflow.  Treat them, rather
    # than model-authored metadata, as the authority for promotion identity.
    inferred_type = _infer_type_from_path(root, candidate_path)
    asset_type = _normalize_asset_type(inferred_type or str(payload.get("asset_type") or ""))
    candidate_id = candidate_path.stem
...
def _system_prompt(asset_type: str) -> str:
    ...
    path = engine_path("templates", "prompts", template)
    if path.exists():
