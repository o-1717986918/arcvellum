...
def _write_state_patch_agent_tasks(
    ...
    return write_agent_tasks(
        ...
        root=root,
        source_paths=[scene_path, source_path, output_path, json_path, root / review_rel],
        notes=[
        ...
        tasks=[
            (
                "审查状态变化依据",
                f"""逐条检查 proposed_updates 是否能从 source_artifact 的可见正文、候选正文或 composition 中找到依据。删除或标记缺少文本证据的状态变化。把依据路径与发现写入 `{review_rel}` 的 evidence_paths 和 findings。""",
            ),
            (
                "审查人物一致性",
            ...
            (
                "决定下一步",
                f"""决定该 patch 应请求用户批准、退回重写、拆分为更小 patch，还是保持候选等待后续场景。不要自动应用。写入 `{review_rel}`：source_artifact={json_path.relative_to(root).as_posix()}、state_patch_sha256（精确 SHA-256）、verdict、approval_recommendation、required_changes；仅当 verdict=pass 且 status=complete 时可完成本任务。""",
            ),
        ],
    )
