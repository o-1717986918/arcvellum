...
from .relay_context import validated_knowledge_quotes, validated_pending_outcome


RELAY_PLAN_SCHEMA = "arcvellum/scene-relay-plan/v1"
...
仅返回 JSON：{{"scene_id":"与 SceneBrief 相同","milestones":[{{"speaker":"participants 中精确名字","source_quote":"SceneBrief 中逐字摘录的既定结果"}}],"actor_knowledge":[{{"speaker":"participants 中精确名字","quotes":["incoming_handoff 中逐字摘录的已发生事实"]}}],"unknown_slots":["尚未确认的关键事实空位"]}}。actor_knowledge 必须覆盖全部 participants；没有额外知情就给空数组。不要输出正文或标准台词。"""
...
def parse_relay_plan(payload: dict[str, Any], brief: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": RELAY_PLAN_SCHEMA, "scene_id": brief["scene_id"],
