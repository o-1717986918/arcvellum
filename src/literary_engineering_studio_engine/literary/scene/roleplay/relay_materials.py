...
def render_relay_materials(
    ...
    block = "\n".join((
        "以下是一级角色 Agent 按真实互动时间顺序生成的非权威素材，不是按人物各写一篇的整场预演。你是唯一正文作者，只可组织、取舍和叙述衔接；所有说出口的话与可见人物行为必须回指对应角色的 entry_id，不得添加演员未生成的台词、动作、手势或操作。保留不同人的句法、避词和互动变化，不把各人的话润平成同一种解释语气。spoken 可作为角色主观话语，不能自动晋升为 Canon；first_person_action 须换成正文视角而不增加动作；private_impulse 只供理解人物，不得直接写入正文。环境候选只供取景，不授权新线索、历史或物证。scene_check 仅证明既定场景结果在候选中有支持，不代表所有候选细节都已证实。若编排时发现来源缺口，报告缺口，不自行补造角色言行。",
