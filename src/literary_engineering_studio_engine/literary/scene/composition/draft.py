...
7. 标点必须符合标准中文标点和文学节奏，不用密集句号、长逗号链、破折号转折或机械转折制造伪节奏。\n8. 精确数字默认不用；动态数值只有在人物需要该精度、数值改变当前选择、模糊后会破坏因果、后文会验证或兑现且同一压力尚未被量化时才进入正文。量词中的数词同样须过五项，普通陈设和日常动作不计件、计次或计秒。身份和连续性所需的年龄、日期、规则编号可保留，但不得重复刷新。
...
8. 不要用脚本化思维处理句子：生成时先避免问题，修订时逐句语义判断；不得把否定、纠偏、讽刺顿挫或人物心理误删成反义。\n9. 交稿前扫描阿拉伯数字、中文数词、序数和量化单位；普通陈设与日常动作的计件、计次、计秒也算。不能说明精确值改变了谁的什么选择，就围绕陈设状态、动作受阻、反复无果、停顿、取用结果或人物可感后果逐句重写，不批量删数字，也不机械替换成模糊量词。
10. 新增事实只列为候选，等待人工确认。
