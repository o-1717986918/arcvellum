...
def build_branch_simulation(
    ...
    if (
        ...
        or not context_trace_status(root, scene_facts.scene_id, context_path).passed
    ):
    ...
    if agent_tasks:
        ...
        roleplay_result = read_semantic_artifact(root, "roleplay-agent-task", scene_facts.scene_id)
    else:
        roleplay_result = {}

    all_cards = _load_characters(root)
    active_cards = _active_cards(all_cards, scene_facts.participants)
    candidates = _build_candidates(scene_facts, active_cards, all_cards, branch_count, roleplay_result)
    ...
    payload = {
        ...
        "roleplay_result": semantic_artifact_relative_path("roleplay-agent-task", scene_facts.scene_id) if agent_tasks else "",
        "roleplay_evidence": roleplay_result,
...
def _build_candidates(
    ...
    return [
        _candidate(branch_id, title, strategy, scene, active_cards, all_cards, roleplay_result or {})
...
def _candidate(
    ...
    action_evidence = _roleplay_strings(roleplay_result.get("character_actions"), limit=2)
    consequence_evidence = _roleplay_strings(roleplay_result.get("world_consequences"), limit=2)
    pressure_evidence = _roleplay_strings(roleplay_result.get("branch_pressures"), limit=2)
    canon_risks = _roleplay_strings(roleplay_result.get("canon_risks"), limit=2)
    if action_evidence:
        action_chain.insert(1, "RP 角色行动依据：" + "；".join(action_evidence))
    if consequence_evidence:
        action_chain.append("RP 世界后果：" + "；".join(consequence_evidence))
    if pressure_evidence:
        action_chain.append("RP 分支压力：" + "；".join(pressure_evidence))

    risks = [*canon_risks, *_risks(branch_id, scene, active_cards, all_cards)]
    ...
    return BranchCandidate(
        ...
        risks=risks,
        writeback_candidates=_writeback(branch_id, scene, lead, roleplay_result),
        scores=scores,
        total_score=total,
        status=status,
    )
...
def _writeback(branch_id: str, scene: SceneFacts, lead: str, roleplay_result: dict[str, object]) -> dict[str, list[str]]:
    label = branch_id.removeprefix("branch_")
    roleplay_writeback = _roleplay_strings(roleplay_result.get("writeback_candidates"), limit=3)
    return {
...
def _roleplay_strings(value: object, *, limit: int) -> list[str]:
    """Flatten bounded Agent evidence without pretending it is formal canon."""
