...
LEGACY_CONTEXT_TRACE_SCHEMA = "literary-engineering-workbench/context-trace/v1"
...
def context_trace_status(root: Path, scene_id: str, context_path: Path | None = None) -> ContextTraceValidation:
    ...
    if payload.get("schema") == LEGACY_CONTEXT_TRACE_SCHEMA:
        return ContextTraceValidation(trace, "stale", "legacy context trace must be rebuilt as v2", payload)
