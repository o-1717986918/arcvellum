...
- 清晰不等于全程平直。过场可以收束句法，冲突转向、关系失衡或认识变化处允许语速、句群、语域和意象升起，再随行动后果回落；这种起伏必须来自场景与人物，不靠随机碎句、形容词或空泛比喻。
...
- 逗号服务尚未完成的动作、观察或因果关系。逗号较多时先检查句内层级，只有关系松散、并列过多或语义重复才重组；不要按数量机械拆成密集短句。句长分布由本场压力与注意力决定，短句留给真实落点，不连续制造鼓点。
- 不做景物强制同步：人物情绪变化时，风、雨、灯、夜色不要恰好配合情绪变化。
- 不要重复渲染同一情绪。同一件事说一遍即可，保留人味和准确细节，不用三个形容词或三个比喻撑篇幅。无关精确数字默认不用，但“一个又一个”等虚指反复不是精确计数；当场问答、人物谈判、事实辨认、选择、因果、连续性或后文核验需要的数值可以保留，不要求所有功能同时成立。技术、灾难、悬疑、倒计时、场景合同和参考语料不能自动授权装饰性读数。普通陈设与日常动作不为显得具体而计件、计次、计秒；删去精度不损失信息和人物反应时，改写为状态、动作受阻或后果。成稿前逐项作语义判断，不批量删数字或机械换成模糊量词。
...
ANTI_AI_STYLE_SHORT_RULE = (
    ...
    "按场景压力调整句法和语域，过场简写，关键转折可有语言起伏；不得用脚本批量删除否定或做语义改写。"
)
