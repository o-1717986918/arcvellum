...
- 数字与量化细节必须采用“默认不用、例外举证”：五项必要性条件缺一即去掉精度，拿不准也按不必要处理；普通时间流逝不算兑现，技术、灾难、悬疑、倒计时、场景合同中的设备读数和参考语料数字都不能豁免。量词中的数词也须过五项，普通陈设和日常动作不计件、计次或计秒。年龄、日期、规则编号只在身份或连续性确有需要时出现。不得把这项规则改写成数字密度阈值、正则扫描器或计数门禁。
- 不摘抄原文，不制造不可验证的作者论断。
...
- 精确数字默认不用，五项必要性条件缺一即去掉精度，拿不准也按不必要处理。动态值只有在人物需要该精度、数值改变当前选择、模糊会破坏因果、后文按该值验证或兑现且同一压力尚未量化时才保留；普通时间流逝不算兑现。
- 年龄、日期、规则编号等身份或连续性事实只在辨认确有必要时出现。技术、灾难、悬疑、倒计时、场景合同中的设备读数和参考语料数字都不能豁免；量词中的数词同样须过五项，电话、灯闪、普通陈设和日常动作不计件、计次或计秒，应改写为陈设状态、动作受阻、反复无果、停顿或取用结果。
- 交稿前扫描阿拉伯数字、中文数词和序数。无法说明精确值改变了谁的什么选择，就围绕动作、状态变化、人物可感范围或后果逐句重写，不批量删除，也不机械替换成模糊量词。
