...
def render_narrative_rhythm_contract(
    ...
    lines = [
        ...
        f"- 密度配比：摘要={density.get('summary', rhythm.get('summary_ratio', 'low'))}，行动={density.get('action', rhythm.get('action_ratio', 'medium'))}，对白={density.get('dialogue', rhythm.get('dialogue_ratio', 'medium'))}，思考={density.get('reflection', rhythm.get('reflection_ratio', 'low'))}，环境={density.get('description', rhythm.get('description_ratio', 'medium'))}",
