...
def _parse_scene_inventory(text: str) -> list[dict[str, object]]:
    ...
    for raw in text.splitlines():
        ...
        if len(cells) < 11:
            # Candidate inventories frequently include a compact ``SC-001 /
            # target / share`` accounting table after the canonical scene
            # table.  It identifies the same scenes but is not a scene
            # contract, so it must not make materialization reject an already
            # complete eleven-field inventory above it.
            if len(cells) <= 3:
                continue
            raise ValueError(f"invalid scene inventory row: {line[:160]}")
        # Do this before ``_number``: its permissive Chinese-number fallback
        # can read an ordinary Chinese header as the number one.
