...
class LeanLongformPlanningServiceTests(unittest.TestCase):
    def test_initial_plan_and_next_chapter_are_resumable_without_legacy_tasks(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("用户指定的章序、终局位置与最终画面优先", gateway.calls[0][1])
            self.assertIn("必须逐字复用 characters", gateway.calls[0][1])
            self.assertIn("既定时间和数值事实保持一致", gateway.calls[0][1])
            self.assertIn("world_facts 只写当前故事确需成立的虚构世界事实与机制", gateway.calls[0][1])
            self.assertIn("章场数量、人物出场范围、视角和语言形式留在规划或用户方向中", gateway.calls[0][1])
            self.assertIn("narrative_design", gateway.calls[0][1])
            self.assertIn("length_weight", gateway.calls[0][1])
            self.assertIn("consequence 可以写条件性的可能变化", gateway.calls[0][1])
            self.assertIn("职业流程只做生活背景", gateway.calls[0][1])
            ...
            self.assertIn("story_time", gateway.calls[1][1])
            self.assertIn("场景详略按事件", gateway.calls[1][1])
    ...
    def test_initial_plan_repairs_an_overfull_first_window_once(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("返修保持用户指定的章序", gateway.calls[1][1])
            self.assertIn("world_facts 只写虚构世界内部的事实与机制", gateway.calls[1][1])
