...
class StoryArchitectureContractTests(unittest.TestCase):
    ...
    def test_revise_verdict_enters_revision_then_requires_fresh_review(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            (root / "plot").mkdir()
            (root / "project.yaml").write_text("target_length: 500000\n", encoding="utf-8")
            ...
            payload = _complete_candidate(candidate)
            ...
            review_payload.update({
                "status": "complete",
                "candidate_sha256": hashlib.sha256(candidate.read_bytes()).hexdigest(),
                "writer_session_id": payload["writer_session_id"],
                "reviewer_session_id": "reviewer-2",
                "verdict": "revise",
                "findings": ["The midpoint does not force the endgame choice."],
                "required_changes": ["Make the midpoint destroy the safe compromise."],
            })
            review.write_text(json.dumps(review_payload), encoding="utf-8")
            write_agent_completion_marker(review_task, root=root)
            ...
            self.assertEqual(validate_task(root, task)[0], [
                "story-architecture revision did not change any declared planning candidate; review-only edits cannot complete revision"
            ])

            payload["midpoint_irreversibility"] = "The safe compromise destroys the sibling's legal identity."
            payload["writer_session_id"] = "writer-3"
            candidate.write_text(json.dumps(payload), encoding="utf-8")
            write_agent_completion_marker(candidate_task, root=root)
            self.assertEqual(validate_task(root, task)[0], [])
            self.assertEqual(_longform_state(root)["current_step"], "story-architecture-review-prepare")

            refreshed, _ = prepare_story_architecture_review(root)
            refreshed_payload = json.loads(refreshed.read_text(encoding="utf-8"))
            self.assertEqual(refreshed_payload["status"], "pending_agent_judgment")
            self.assertEqual(
