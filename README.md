...
当前版本：**v0.9.1 Spatial Orrery Navigation & Release Guard**。

![ArcVellum 叙事星仪与规则节奏仪表](docs/images/arcvellum-orrery-v080.png)
...
v0.9.1 在 v0.9.0 的空间场域与正式节奏闭环上，强化了全书、章节与场景三级的平滑叙事连接：章节以明确的场景承接曲线呈现，单场景以人物、分支、审查、设定、承诺和工作流的关系曲线呈现。桌面构建同时新增冻结 sidecar 来源证明，直接 Tauri 打包或来源过期时会被阻断。它仍属于 Beta：进入 v1.0 前还需持续积累多类型长篇样例、长时间无人值守恢复证据、Windows 10/11 干净虚拟机矩阵与商业代码签名验证。
