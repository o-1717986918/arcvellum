...
当前版本：**v0.9.2 Desktop Startup Reliability Hotfix**。
...
v0.9.2 修复了冻结桌面 sidecar 经由动态模块路径启动而导致本地服务无法加载的问题；现在 API 应用在冻结进程内直接创建，并且桌面加载页会显示 sidecar 启动失败的诊断信息，而不再无限等待。它仍属于 Beta：进入 v1.0 前还需持续积累多类型长篇样例、长时间无人值守恢复证据、Windows 10/11 干净虚拟机矩阵与商业代码签名验证。
