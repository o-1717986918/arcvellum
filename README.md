...
当前版本：**v0.8.0 Creative Control & Narrative Orrery**。

![ArcVellum v0.8 叙事星仪与规则节奏仪表](docs/images/arcvellum-orrery-v080.png)
...
首页不再是一组平均用力的状态卡。Narrative Orrery 以真实作品资料绘制章节长卷、人物轨迹、候选分支、审查债务、Canon 锚点和当前任务；场景形成、分支出现、正文增长与设定写回会触发对应动画。大型作品在全书视图按章节聚合，场景细节按需展开，并保留键盘列表和减少动态模式。

星仪提供工作台与全视口沉浸两种模式。沉浸模式不是功能缩水的屏保：项目生命体征、当前任务、自动推进、人工决策、创作规则与叙事节奏、正式正文和路线健康度都保留在四侧控制坞与原生仪表中。仪器窗可以多开、折叠、拖动、置顶和复位；默认四区网格会为标题、场景选择和中央故事脉络保留安全空间。苔夜星仪、靛紫航图、黑曜黄铜、米白书柜和冷峻现代五套主题分别拥有独立生成式背景、互补语义色与半透明材质，而不是只改一个强调色。所有节点和动效仍来自项目真实状态，背景不承载虚假数据。
...
v0.8.0 在 v0.7.1 安装版连接热修之上完成创作品质档案、Narrative Projection v2、完整沉浸工作台、受控自然语言项目控制台、角色隔离的持久 Agent Runtime、同会话预检修复、实时事件与读取缓存。它仍属于 Beta：进入 v1.0 前还需持续积累多类型长篇样例、长时间无人值守恢复证据、Windows 10/11 干净虚拟机矩阵与商业代码签名验证。
