...
const props = defineProps<{ identity?: ArtifactIdentity; characters?: number; kind?: string }>();
...
<template>
  <section class="creative-identity-rail" :data-identity="identity || 'waiting'">
    <header><span>内容状态</span><strong>{{ Number(characters || 0).toLocaleString('zh-CN') }} 字符</strong></header>
    <ol>
    ...
    <p v-else><Sparkles :size="13" />{{ kind === 'prose' ? '正文通过审查与晋升后进入阅读长卷。' : '内容通过检查后会进入正式项目资产。' }}</p>
  </section>
</template>
