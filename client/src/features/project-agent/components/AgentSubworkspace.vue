...
import type { ProjectAgentWorkspaceDescriptor } from "@/features/project-agent/workspaces";
...
<template>
  <section class="pa-subworkspace" :aria-label="workspace.title">
    ...
    <div class="pa-subworkspace-scroll">
      <Suspense>
        <KeepAlive :max="9">
          <component :is="workspace.component" :key="`${workspace.id}:${revision}`" />
        </KeepAlive>
        <template #fallback>
          <div class="pa-subworkspace-state"><i></i><strong>正在打开{{ workspace.title }}</strong><span>作品数据会继续留在当前桌面中。</span></div>
        </template>
      </Suspense>
    </div>
  </section>
